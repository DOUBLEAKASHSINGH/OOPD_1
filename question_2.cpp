...
class Tax_calculator {
    ...
    public:
        ...
        double selling_price_2024(){
            double sp = purchased_price;
            for (int i = 0; i < no_of_records - 1; i++) {
                if (stoi(csv_data[i][0]) > purchased_year && stoi(csv_data[i][0]) <= selling_year && stoi(csv_data[i][0]) < 2024) {                    
                    sp *= (1 + (stod(csv_data[i][1]) - stod(csv_data[i][2]))/100 );
                }
                if (stoi(csv_data[i][0]) > purchased_year && stoi(csv_data[i][0]) <= selling_year && stoi(csv_data[i][0]) >= 2024) {                    
                    sp *= (1 + stod(csv_data[i][1])/100 );
                }
            }
            return sp;
        }

        double lctg_tax_2024() {
            double sp = selling_price_2024();
            double profit = sp - purchased_price;
            double tax = 0;
            for(int i=0; i<no_of_records-1; i++)
            {
                if(stoi(csv_data[i][0]) < 2024)
                {
                    tax = 0.2 * profit;
                    return tax <= 0 ? 0 : tax;
                }
...
int main() {
    ...
    cout << "Selling Price: " << fixed << tax.selling_price_2024() << endl;
    cout << "Tax: " << fixed << tax.lctg_tax_2024();
    return 0;
}
